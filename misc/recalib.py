...
def chessboard_detect(rgb, K, dist_coef, pattern_size=(8, 5), square_size=0.025):
    ...
    cv2.putText(img, f"Pitch: {pitch:.2f} degrees", (20, 40), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    cv2.putText(img, f"Yaw: {yaw:.2f} degrees", (20, 80), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    cv2.putText(img, f"Roll: {roll:.2f} degrees", (20, 120), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
...
def recalib(im: np.ndarray, intri: dict, extri: dict):
    ...
    print(f"ground truth roll is {roll} degrees")
...
if __name__ == "__main__":
    ...
    angles, corners = chessboard_detect(rgb, np.array(intri["cam_intrinsic"]).reshape((3, 3)), np.array(intri["cam_distcoeffs"]))
